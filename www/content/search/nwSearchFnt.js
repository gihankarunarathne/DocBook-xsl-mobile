  ...
  if (finalWordsList.length) {
    ...
    for (var i = 0; i < cpt; i++) {
      ...
      if (fileAndWordList[i] != undefined) {
        ...
        //mobile: linkTab.push("<ul class='searchresult'>");
        linkTab.push("<ul data-role='listview' data-filter-theme='d' data-theme='d' data-divider-theme='b' data-inset='true' data-inset='true'>");
        for (t in fileAndWordList[i]) {
          ...
          var linkString = '<li><a id="' + idLink + '" href="' + tempPath + '" class="">' + tempTitle + '</a>';
          ...
          /*mobile: if ((tempShortdesc != "null" && tempShortdesc != '...')) {
...
*/
  ...
  if (verifyBrowser()) {
    //mobile: document.getElementById('searchResults').innerHTML = results;
    $.get('#searchResults',function(){
      $('#searchResults').append(results).trigger('create');
    },'html');
  } else {
    //mobile: document.getElementById('searchResults').innerHTML = warningMsg + results;
    $.get('#searchResults',function(){
      $('#searchResults').append(warningMsg + results).trigger('create');
    }, 'html');
  }
...
function verifyWord(word, arr) {
  for (var i = 0; i < arr.length; i++) {
    if (word[0] == arr[i][0] && word[1] == arr[i][1]
    //&& word[2] == arr[i][2]
    ) {
      return true;
    }
...
function SortResults(mots) {
  ...
  for (var i = 0; i < listToOutput.length - 1; i++) {
    for (var j = i + 1; j < listToOutput.length; j++) {
      if (listToOutput[i][0].motsnb < listToOutput[j][0].motsnb || (listToOutput[i][0].motsnb == listToOutput[j][0].motsnb && listToOutput[i][0].scoring < listToOutput[j][0].scoring)) {
